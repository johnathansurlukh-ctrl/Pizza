...
function PizzaSVG({ build }) {
  const toppingPositions = [
    [50, 40], [30, 55], [70, 55], [50, 70], [25, 38], [75, 38],
    [35, 72], [65, 72],
  ]
  const activeTop = build.toppings || []
  const sauceColors = { classic: '#e85d04', spicy: '#dc2626', garlic: '#fef9c3', bbq: '#7c2d12' }
  const cheeseOpacity = { mozzarella: 0.6, cheddar: 0.7, extra: 0.8, four: 0.9 }
  ...
  return (
    <svg viewBox="0 0 100 100" className="w-full h-full drop-shadow-2xl">
      ...
      <circle cx="50" cy="50" r="38"
        fill={sauceColors[build.sauce] || '#e85d04'}
        opacity="0.9" />
      {/* Cheese layer */}
      {build.cheese && (
        <circle cx="50" cy="50" r="36"
          fill={build.cheese === 'cheddar' ? '#fbbf24' : '#fef3c7'}
          opacity={cheeseOpacity[build.cheese] || 0.6} />
      )}
      {/* Toppings */}
      {activeTop.slice(0, 8).map((t, i) => {
        ...
        return (
          <text key={t} x={x} y={y} fontSize="8" textAnchor="middle" dominantBaseline="middle">
            {opt?.emoji || '●'}
          </text>
        )
      })}
      {/* Shine */}
      <ellipse cx="38" cy="35" rx="8" ry="5" fill="white" opacity="0.08" transform="rotate(-20 38 35)" />
      {/* Stuffed crust visual */}
      {build.base === 'stuffed' && (
        <circle cx="50" cy="50" r="46" fill="none" stroke="#fbbf24" strokeWidth="4" opacity="0.5" strokeDasharray="8 4" />
      )}
    </svg>
  )
}
...
export default function Builder() {
  ...
  const handleAddToCart = () => {
    ...
    const selectedSauce = options.sauce.find(o => o.id === build.sauce)
    addItem({
      id: `custom-${Date.now()}`,
      ...
      ingredients: [selectedBase?.label, selectedSauce?.label, ...build.toppings],
      rating: 5,
    })
    setAdded(true)
                ...
                {activeStep !== 'toppings' && options[activeStep].map(opt => (
                  ...
                  >
                    ...
                    <div className="flex-1 text-left">
                      ...
                      <div className="text-xs text-white/40">{opt.cal} kcal{opt.price > 0 ? ` · +₹${opt.price}` : ''}</div>
                    </div>
              ...
              <div className="space-y-3 mb-5 text-sm">
                ...
                {build.toppings.length > 0 && (
                  <div className="flex items-start justify-between">
                    <span className="text-white/50">Toppings ({build.toppings.length})</span>
                    <div className="text-right">
                      {build.toppings.map(t => {
                        const opt = options.toppings.find(o => o.id === t)
                        return <div key={t} className="text-white font-medium text-xs">{opt?.emoji} {opt?.label}</div>
                      })}
                    </div>
                  </div>
                )}
